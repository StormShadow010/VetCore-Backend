...
export const getMascotas = async (req: Request, res: Response) => {
  ...
  const user = (req as typeof req & { user?: { sub: number; email?: string } }).user;
  ...
  try {
    const r = await query(`${MASCOTA_WITH_JOINS} ${where} ORDER BY m.activa DESC, m.nombre`, params);
    ok(res, r.rows);
...
export const createMascota = async (req: Request, res: Response) => {
  const { nombre, id_especie, raza, sexo, fecha_nac, peso_kg, color } = req.body;
  ...
  if (user?.rol === 'USUARIO' && !id_propietario) {
    ...
    const userResult = await query('SELECT email FROM usuarios WHERE id_usuario = $1', [user.sub]);
    ...
    if (!id_propietario) {
      ...
      const userInfo = await query(
        'SELECT username, email FROM usuarios WHERE id_usuario = $1', [user.sub]
      );
      if (userInfo.rows[0] && userInfo.rows[0].email) {
        ...
        res.status(400).json({ success: false, message: 'Tu cuenta no tiene email registrado. Contacta al administrador.' });
        return;
      }
    }
  }
...
export const updateMascota = async (req: Request, res: Response) => {
  const { id } = req.params;
  const { nombre, id_especie, raza, sexo, fecha_nac, peso_kg, color, id_propietario, activa } = req.body;
  try {
    const r = await query(
      ...
      [nombre, id_especie, raza, sexo, fecha_nac, peso_kg, color, id_propietario,
       activa !== undefined ? activa : null, id],
    );
    if (!r.rows[0]) { notFound(res, 'Mascota no encontrada'); return; }
    ok(res, r.rows[0]);
  } catch (e) { logger.error(e); serverError(res); }
...
export const hardDeleteMascota = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM tratamientos WHERE id_consulta IN (
      ...
      JOIN citas c ON c.id_cita = co.id_cita WHERE c.id_mascota = $1)`, [id]);
    await client.query(`DELETE FROM facturas WHERE id_cita IN (
      SELECT id_cita FROM citas WHERE id_mascota = $1)`, [id]);
    await client.query(`DELETE FROM consultas WHERE id_cita IN (
      SELECT id_cita FROM citas WHERE id_mascota = $1)`, [id]);
    await client.query('DELETE FROM citas WHERE id_mascota = $1', [id]);
    const r = await client.query('DELETE FROM mascotas WHERE id_mascota = $1 RETURNING nombre', [id]);
    await client.query('COMMIT');
    if (!r.rows[0]) { notFound(res, 'Mascota no encontrada'); return; }
    ok(res, { message: `Mascota "${r.rows[0].nombre}" eliminada permanentemente` });
  } catch (e) {
